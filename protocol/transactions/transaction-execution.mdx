...
A final `Receipt` is processed in a new block, refunding any extra gas paid by the user. Almost every transaction ends with one of these, since the price gas is [bought at](./gas#buying-vs-burning-gas) is usually higher than the price it burns at.
