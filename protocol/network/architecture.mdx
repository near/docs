...
Similar to Ethereum, Near Protocol is an [account-based system](/protocol/accounts-contracts/account-model). Which means that each blockchain user is roughly
associated with one or several accounts (there are exceptions though, when users share an account and are separated
through the [access keys](/protocol/accounts-contracts/access-keys)).
...
inside the runtime layer together with the [gas](/protocol/transactions/gas), [fees](/protocol/transactions/gas#understanding-gas-fees), and rewards.
...
- [Fees and rewards](/protocol/transactions/gas) -- fees and rewards are neatly encapsulated in the runtime layer. The blockchain layer, however,
