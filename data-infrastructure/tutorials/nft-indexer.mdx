...
This tutorial will guide you through building a simple NFT indexer using the JavaScript version of the [NEAR Lake Framework JS](https://www.npmjs.com/package/near-lake-framework). The indexer will listen for `nft_mint` events and print relevant data about newly minted NFTs.
...
We will use JS version of [NEAR Lake Framework](https://www.npmjs.com/package/near-lake-framework) in this tutorial. Though the concept is the same for Rust, but we want to show more people that it's not that complex to build your own indexer.
...
Please, ensure you've the credentials set up as described in the [development environment setup](/data-infrastructure/tutorials/running-near-lake/run-near-lake#prepare-development-environment). Otherwise you won't be able to get the code working.
...
Having troubles running the indexer? Please, check you haven't skipped the [development environment setup](/data-infrastructure/tutorials/running-near-lake/run-near-lake#prepare-development-environment) part :)
...
Mintbase turn, we hope [Nate](https://twitter.com/nategeier) and his team have migrated to [NEAR Lake Framework](https://www.npmjs.com/package/near-lake-framework) already, saying "Hi!" and crafting the link:
