...
Query the transaction status. The difference between this endpoint and `tx` endpoint is that this endpoint returns,
in addition to receipt outcomes, the receipts themselves.
...
- params
 ...
 ```bash
 http post <node_url> jsonrpc=2.0 method=EXPERIMENTAL_tx_status params:='["HEgnVQZfs9uJzrqTob4g2Xmebqodq9waZvApSkrbcAhd", "bowen"]'' id=dontcare
 ```
