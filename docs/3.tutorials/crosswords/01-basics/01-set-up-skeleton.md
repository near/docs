...
You may have found the [online Rust Book](https://doc.rust-lang.org/stable/book), which is a great resource for getting started with Rust. However, there are key items that are different when it comes to blockchain development. Namely, that smart contracts are [technically libraries and not binaries](https://learning-rust.gitbook.io/book/basics/cargo-crates-and-basic-project-structure#crate), but for now just know that we won't be using some commands commonly found in the Rust Book.
