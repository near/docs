...
Three examples on how to handle updates and [state migration](../../2.develop/contracts/anatomy.md):
...
:::tip
Notice that migrate is actually an [initialization method](../../2.develop/anatomy.md#initialization-method) that ignores the existing state ([#init(ignore_state)]), thus being able to execute and rewrite the state.
:::
