...
- `near` (the connection object returned by a call to `near-api-js.connect()`)
...
- `near-api-js.Account` represents an account on the NEAR platform
- `near-api-js.Connection` represents a connection to the NEAR platform
- `near-api-js.Contract` represents a Smart Contract on the NEAR platform
- `near-api-js.InMemorySigner` is used for signing transactions before they're sent to the network
- `near-api-js.KeyPair` is used to generate new key pairs
- `near-api-js.WalletAccount` is used to instantiate a `wallet` (as per the big 3 above)

**utility collections**
- `near-api-js.keyStores` surfaces various key stores including
...
- `near-api-js.transactions` surfaces transaction factory methods (ie. `near-api-js.transactions.addKey()` produces a hydrated `AddKey` transaction ready for serialization and signing before being sent to the network) and their parameters (ie. `FunctionCallPermission` is a type of `AccessKey` that may be used as a parameter to the `addKey()` method)
- `near-api-js.utils` surfaces various utility classes and functions including
...
- `near-api-js.providers` surfaces access to the `JsonRpcProvider` which wraps the RPC interface

**methods**
- `near-api-js.connect(config)` returns a `near` connection object given a configuration object as per the [introduction to `near-api-js`](/docs/roles/developer/examples/near-api-js/introduction)
- `near-api-js.providers.getTransactionLastResult(fxo)` returns the result of the last transaction given a `FinalExecutionOutcome` object (the value returned after sending a signed transaction to the network)

<br>

### The `near` Connection Interface

This is an instance of the `near-api-js.Connection` class ([view source](https://github.com/near/near-api-js/blob/master/src/connection.ts))
...
This is an instance of the `near-api-js.WalletAccount` class ([view source](https://github.com/near/near-api-js/blob/master/src/wallet-account.ts))
...
let decodedTxHash = near-api-js.utils.serialize.base_decode("3L37n7LowxfpFEJcQvYmoQPo4NpgV2ZUmr4vXSBYhxPL")
