...
So, when looking for a feature of `nearlib` in the source code, know that the `src` folder is the authority for functionality and `test` contains the code that exercises all of this functionality.  The rest of the repo can (should?) be safely ignored.
