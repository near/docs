...
<blockquote class="warning">
<strong>work in progress</strong> <span>flush out transaction class</span><br><br>

- intro
- interface
- link to GitHub ([here](https://github.com/nearprotocol/nearlib/blob/master/src.ts/transaction.ts))
...
the [`Near`](https://github.com/nearprotocol/nearlib/blob/master/src.ts/near.ts) class exposes several methods including this one:

`async function connect(config: any): Promise<Near> `
...
(async () => {
  ...
  window.config = {
    networkId: 'default',
    // contractName: CONTRACT_NAME, // optional contract name
...
- <span class="error">`Failed to load master account key from ${config.keyPath}: ${error}`</span>
  if you see an error like this it means that no account was found at the expected path and you may need to authenticate (via `wallet.requestSignIn()` on the client side or via NEAR Shell `near login` on the server side)

### connecting to the NEAR Wallet

```js
// if no local wallet account is found, set one up
// via new account or account recovery
if(!window.walletAccount.isSignedIn()) {
  // instantiate a new wallet
  window.wallet = new nearlib.WalletAccount(window.near);

  // initiate NEAR Wallet app authorization flow
...
// users can also sign out of NEAR Wallet
function logout(){
...
<blockquote class="warning">
<strong>work in progress</strong> <span>RPC interface</span><br><br>

- start with generic RPC query interface `async query(path: string, data: string): Promise<any> {}`
- cover a few examples such as

```js
// supported method provider.query
await account.connection.provider.query(`account/${accountId}`, '');
await account.connection.provider.query(`access_key/${accountId}`, '');
await account.connection.provider.query(`access_key/${accountId}/${publicKey.toString()}`, '');
await account.connection.provider.query(`call/${contractId}/${methodName}`, base_encode(JSON.stringify(args)));

// unsupported method provider.sendJsonRpc (designated as private in TypeScript)
await near.connection.provider.sendJsonRpc('query', [path, data]);
await near.connection.provider.sendJsonRpc('block', [blockId]);
await near.connection.provider.sendJsonRpc('chunk', [chunkId]);
await near.connection.provider.sendJsonRpc('broadcast_tx_commit', [Buffer.from(bytes).toString('base64')]);
await near.connection.provider.sendJsonRpc('tx', [base_encode(txHash), accountId]).then(adaptTra
```

</blockquote>

### fetch an account from the NEAR platform
...
```js
(async () => {
  const account = await near.account("some-valid-account-name")
  console.log("account", account)
})()
```

<blockquote class="warning">
<strong>work in progress</strong> <span>add other parts of the interface</span><br><br>
...
- <span class="error">Must specify account creator, either via masterAccount or helperUrl configuration settings.</error>

## create a transaction

<blockquote class="warning">
<strong>work in progress</strong> <span>cover various types of transactions</span><br><br>
...
nearlib.transactions.deleteAccount
nearlib.transactions.deleteKey
nearlib.transactions.deployContract
nearlib.transactions.functionCall
nearlib.transactions.stake
nearlib.transactions.transfer
```

sample parameters:

```js
nearlib.transactions.AccessKey
nearlib.transactions.FullAccessPermission
nearlib.transactions.FunctionCallPermission
```

</blockquote>

<blockquote class="warning">
<strong>work in progress</strong> <span>cover adding transaction to a collection of actions</span><br><br>
...
The Account object you use to sign and send transactions
- must exist on the network identified by `config.nodeUrl`
- must be under your control as proven by your access to at least one private key matching one of the public access keys (`FullAccess` or `FunctionCall` access) previously configured on the account
- must have enough funds in its account to cover the costs of processing the transactions and related storage costs on the network

</blockquote>

<blockquote class="warning">
<strong>work in progress</strong> <span>cover basics of signing and verifying signatures</span><br><br>
...
<blockquote class="warning">
<strong>work in progress</strong> <span>cover key stores</span><br><br>

```js
await signer.keyStore.setKey(networkId, accountId, keyPair);
await signer.keyStore.getKey(networkId, accountId);
```
...
```js
window.near.sendTokens(BN(1000), "valid-sender-account", "valid-receiver-account")
```

## interacting with contracts

### load a contract from the NEAR platform to interact with it
...
window.contract = await near.loadContract(config.contractName, {
  viewMethods: ['method1', 'method2'],
  changeMethods: ['method3'],
  sender: window.accountId,
});
```

see the [`Contract`](https://github.com/nearprotocol/nearlib/blob/master/src.ts/contract.ts) class constructor for details

### call a contract method
```js
window.contract.method1().then((returnVal) => {
  console.log("method1 returned", returnVal)
});
```

This will allow you to use any contract for which you know (a) the contract name, (b) at least one `view` or `change` method and (c) have an account ID which is authorized to invoke those methods.

The object returned is populated with `view` and `change` methods as attributes which can be invoked like any other JavaScript method including parameters and return values, except execution site is on the NEAR blockchain (in an ephemeral virtual environment not unlike serverless functions) and results appear locally as soon as they are returned by the contract method.
