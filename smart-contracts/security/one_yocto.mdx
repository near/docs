...
NEAR uses [an access key system](../../protocol/accounts-contracts/access-keys) to simplify account management. There are two main types of keys:

### 1. Full Access Keys
- **Full control** over an account
- Can perform all [actions](../anatomy/actions) (transfers, deployments, etc.)
...
When a user [signs in to a website](../../web3-apps/tutorials/wallet-login) to interact with your contract:
...
<Tip>
[`near_sdk`](../../tools/sdk) provides a helper method to assert one yoctoNEAR deposit: 
