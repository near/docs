...
**Go contracts** use a two-tier testing approach:

- **Unit tests** run locally via `near-go test project`, using the `MockSystem` environment to simulate the NEAR blockchain — no sandbox required.
- **Integration tests** that deploy to a sandbox are written in Rust using the `workspaces-rs` framework. See the [Go Integration Testing](#go-integration-testing) section below.
...
<Note>
Go integration tests are written in Rust using the `workspaces-rs` framework. The Go contract is compiled to WASM first, then the Rust test suite deploys and interacts with it via the sandbox. See the [Go Integration Testing](#go-integration-testing) section for setup details.
</Note>

    <Github fname="test_greeting.rs"
              url="https://github.com/Emir-Asanov/near-go-examples/blob/example-release-1/greeting/integration-tests/tests/test_greeting.rs" start="1" end="77"/>
  </Tab>
</Tabs>
...
Go integration tests are written in Rust using the `workspaces-rs` framework. The Go contract is compiled to WASM first, then the Rust test suite deploys and interacts with it via the sandbox. See the [Go Integration Testing](#go-integration-testing) section for setup details.
    ...
    <Github fname="test_donation.rs"
              url="https://github.com/Emir-Asanov/near-go-examples/blob/example-release-1/donation/integration-tests/tests/test_donation.rs" start="1" end="81"/>
  </Tab>
</Tabs>
