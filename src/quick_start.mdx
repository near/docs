...
This is exactly what currently runs the DevNet that supports apps deployed using the NEAR Studio IDE (though that DevNet is hosted in the cloud by us).
...
After following the instructions above to set up Rust, download the source code, build and then run DevNet:
...
Use the python commands below to try submitting transactions or views via [JSON RPC](https://www.jsonrpc.org/specification).
...
There is a Python script that wraps the RPC:
...
*Note: This is advanced functionality which may be more brittle than usual. Proceed at your own peril.*
