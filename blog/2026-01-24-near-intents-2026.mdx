...
Given the vast amount of chains and assets that have surged, it is now common for users to need to swap assets across chains (e.g. swap USDT on Ethereum for SOL on Solana).
...
But before, let's briefly review how cross-chain swaps have traditionally worked, and what challenges they faced, to better understand why NEAR's approach is unique and powerful.
...
### How MPC, the bridge, and intents fit together
...
In short: **intents orchestrate the swap, the bridge moves the assets, and the MPC Network is the cryptographic engine that lets the bridge act on other chains.**
...
While MPC is a powerful primitive that exists on other chains, in NEAR it is naturally integrated into the on-chain execution thanks to the fact that smart contracts can yield and resume computation.
