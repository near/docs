...
Between the `yield` and `resume` the user will simply be waiting to receive the result. The maximum time to wait is 200 blocks before the protocol kills the `yield` - around 4 minutes.
...
pub fn request_weather(&mut self, city: String) {
    ...
    // The return for this function will be the result of "callback_return_result" 
    // defined bellow as `pub fn callback_return_result`
