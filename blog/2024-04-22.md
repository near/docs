...
Besides checking broken links, we took the time to make sure all the **translations were correctly migrated**. The system that docusaurus uses (called [crowdin](https://crowdin.com/)) is not very good on detecting changes on a file. Migrating all the translations was a huge effort on itself. 
