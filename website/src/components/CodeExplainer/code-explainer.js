...
export function ExplainCode({ children, languages }) {
  ...
  // validate languages
  if (!languages.every((lang) => lang in lang2label))
    throw new Error("languages must be one of ['rust', 'js', 'ts']");
      ...
      if (child.props.highlights) {
        ...
        if (props_highlights && language in props_highlights) {
          blocks.push({
            ...
            highlight: props_highlights[language],
...
export function InnerBlock({ selected, text, index, activateFn, type }) {
  ...
  return (
    <div
      className={`block ${cssState} padding--sm`}
      key={index}
      id={`block${index}`}
      onClick={() => activateFn(index)}
    >
      {text}
    </div>
  );
