...
A nice way to learn is by trying to expand the contract. Modify it so that you store one greeting message **per user**. For this, you will need to use knowledge from the [environment](../../2.build/2.smart-contracts/anatomy/environment.md) and [storage](../../2.build/2.smart-contracts/anatomy/storage.md) sections. You can also use the [guest book](guest-book.md) example, since it does something similar.
