...
## Các status của Node {#node-status}
...
**Lưu ý:** Đối với `["block hash"]` & `[block number]` bạn cần phải query các block mới, vì chúng sẽ bị garbage collect sau năm [epoch](/docs/concepts/epoch).
...
## Status của quá trình xác thực {#validation-status}
