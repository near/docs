...
No. Chúng tôi xử lý các rủi ro đối với mạng lưới ở mức protocol level. For example, we have a lot of limiters that constrain how much data you can pass into a function call or how much compute you can do in one function call, etc.

That said, smart contract developers will need to be responsible for their own dApps, as there is no stage gate or approval process. Tất cả các lỗ hổng chỉ có thể gây nguy hiểm đến bản thân smart contract. May mắn là, việc update các smart contract trên NEAR rất đơn giản, vì thế các lỗ hổng có thể được vá nhanh gọn, không giống như trên các blockchain khác.
</blockquote>
